...
const editingRowKey = defineModel<RowKey>('editingRowKey')
provide('editingRowKey', editingRowKey)
...
const { visibleItems, createChild, createSibling, _insInfo } = useVisibleItems(
    props.keyField,
    props.parentIdField,
    rawItems,
    expandedRowKeys,
...
function onKeyDown(event: KeyboardEvent) {
    switch (event.key) {
        case 'Escape':
            ...
            break
        case 'Enter':
            if (focusedRowKey.value) createSibling(focusedRowKey.value)
            break
        case 'ArrowUp':
            focusPrevRow()
...
function focusPrevRow() {
    const ci = visibleItems.value.findIndex(
        (i) => i.key === focusedRowKey.value
    )
    let ni: number
    if (ci === 0) {
...
<template>
    ...
    >
        <table class="sy-tree-table__table">
            ...
            <tbody>
                <sy-tree-rows> </sy-tree-rows>
            </tbody>
        </table>
        <textarea v-model="visibleItemDbg"> </textarea>
    </div>
</template>
